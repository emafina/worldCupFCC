...
echo "$($PSQL "SELECT name FROM (SELECT winner_id AS id FROM games WHERE year=2014 AND round = 'Eighth-Final' UNION SELECT opponent_id AS id FROM games WHERE year=2014 AND round = 'Eighth-Final') AS list LEFT JOIN teams ON list.id=teams.team_id ORDER BY name")"
